...
$(document).ready(() => {
  ...
  app.titleChanger(['Bloom', 'Real Nigga', 'Auth Intro Be Like :sus:']);
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Authenticating...', "Granting access to <span style='font-size: 14px; color: #06d;'>[unknown_user]</span>..."], 30, () => {
    ...
    const state = data.state_prov ? data.state_prov : 'your state';

    writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Welcome back, <i style='color: #0f0'>${ip}</i>! By the way, nice to see someone from ${state} in ${country} here!`], 30, 500, () => {
      if (app.skippedIntro) return;

      clearCursor();

      writeLine([`<i style='color: #F62459'>Bloom $$$</i>`], 120, 500, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
  ...
  $('#main').fadeOut(100, () => {
    ...
    setTimeout(() => {
      ...
      $('.background').fadeIn(200, () => {
        if (!app.shouldIgnoreVideo) $('#audio').animate({ volume: 600 }, app.musicFadeIn);
